...
\begin{equation*}
    \hat{\sigma}^2=\frac{1}{p-2r}\sum_{i=2r+1}^{p} \lambda_i(S).
\end{equation*}
...
In this section, we derive some properties of the new test statistic. Our main results require the following relationship of $n$ and $p$.
...
First we establish $\hat{\sigma}^2$ is a consistent estimator of $\sigma^2$. 
\begin{proposition}\label{varianceEstimation}
    Under Assumptions~\ref{balance}-\ref{theModel2},      $\hat{\sigma}^2$ is consistent.
\end{proposition}

Now we derive the asymptotic normality of the new test statistic. Consider the case  when the eigenvalues of $\Sigma$ is bounded, i.e., there is no clear correlation between variables.
...
\begin{corollary}\label{maxEigen}
    Suppose that $W_n$ is a $p \times p$ matrix distributed as $\mathrm{Wishart}_p(n,I_{p})$, $p/n\to c\in [0,+\infty]$. Then as $n\to \infty$,
    $$
...
\begin{proof}[\textbf{Proof}]
    ...
    $$
    \frac{\lambda_1(W_n)}{n}\xrightarrow{P}{(1+c)}^2.
    $$
    Hence the conclusion holds. If $c=+\infty$, suppose $W_n=Z_n Z_n^T$ where $Z_n$ is a $p\times n$ matrix with all elements distributed as $N(0,1)$. Then
    $$
    \frac{\lambda_1(W_n)}{p}=\frac{Z_n^T Z_n}{p}\xrightarrow{P} 1,
    $$
    by Lemma~\ref{baiyin}, which proves the conclusion.
\end{proof}
