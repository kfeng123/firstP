...
where $R$ is a hyperparameter.
    ...
    Therefore, without loss of generality, we will assume that $r$ is known throughout the paper.
...
\end{equation*}
where
$S_k={(n_k -1)}^{-1}\sum_{i=1}^{n_k} (X_{ki}-\bar{X}_k) {(X_{ki}-\bar{X}_k)}^T
$
...
\begin{remark} When both samples are simultaneously transformed by shift and orthogonal transformation, the statistic $T_2$ is invariant.
    ...
    \begin{equation*}
        \textrm{$X_{1i}\mapsto OX_{1i}+\mu$ and $X_{2j}\mapsto OX_{2j}+\mu$, $i=1,\ldots,n_1$, $j=1,\ldots,n_2$,}
    \end{equation*}
...
\begin{theorem}\label{chilimthm}
    Suppose Assumptions~\ref{balance}-\ref{theModel2} hold and $\kappa=1$.
